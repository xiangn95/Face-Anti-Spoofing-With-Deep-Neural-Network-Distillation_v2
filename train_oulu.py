...
def train_parent():
    ...
    for epoch in range(args.epochs):  # loop over the dataset multiple times
        ...
        teacher_train_data = oulu_npu_dataset_train(teacher_train_list, teacher_image_dir, transform=transforms.Compose([RandomErasing(), RandomHorizontalFlip(), Resize(256), CenterCrop(224), ToTensor(), Cutout(), Normaliztion()]))
    ...
    for epoch in range(args.epochs):  # loop over the dataset multiple times
        ...
        for i, sample_batched_teacher in enumerate(teacher_dataloader_train):
        	for j, sample_batched_student in enumerate(student_dataloader_train):
	            ...
	            # set_trace()
	            accu.append(accuracy(output_2, spoof_label_s.long().squeeze(1))[0].item())
