...
from oulu_train import oulu_dataset_train, Normaliztion, Resize, CenterCrop, ToTensor, RandomHorizontalFlip, Cutout, RandomErasing
from oulu_val import oulu_dataset_val, Resize_val, CenterCrop_val, Normaliztion_valtest, ToTensor_valtest

from casia_fasd_train import casia_fasd_dataset_train
from casia_fasd_val import casia_fasd_dataset_val
...
# Dataset root
teacher_image_dir = '/data1/xiangyaohuang/oulu/'         
student_image_dir = '/data1/ziqitang/CBSR/' 

teacher_train_list = '/data1/xiangyaohuang/oulu/Protocols/Protocol_4/Train_1.txt'
student_train_list = '/data1/xiangyaohuang/train_label.txt'

teacher_val_list = '/data1/xiangyaohuang/oulu/Protocols/Protocol_4/Dev_1.txt'
student_val_list = '/data1/xiangyaohuang/test_label.txt'
...
def train_parent():
	...
	for epoch in range(args.epochs):  # loop over the dataset multiple times
		...
		teacher_train_data = oulu_dataset_train(teacher_train_list, teacher_image_dir, transform=transforms.Compose([RandomErasing(), RandomHorizontalFlip(), Resize(256), CenterCrop(224), ToTensor(), Cutout(), Normaliztion()]))
		...
		with torch.no_grad():
			...
			###########################################
			# val for threshold
			val_data = oulu_dataset_val(teacher_val_list, teacher_image_dir, transform=transforms.Compose([Resize_val(256), CenterCrop_val(224), Normaliztion_valtest(), ToTensor_valtest()]))
...
def train_student():
	...
	for epoch in range(args.epochs):  # loop over the dataset multiple times
		...
		# load random 16-frame clip data every epoch
		teacher_train_data = oulu_dataset_train(teacher_train_list, teacher_image_dir, transform=transforms.Compose([RandomErasing(), RandomHorizontalFlip(),  ToTensor(), Cutout(), Normaliztion()]))
		teacher_dataloader_train = DataLoader(teacher_train_data, batch_size=args.batchsize, shuffle=True, num_workers=0)

		student_train_data = casia_fasd_dataset_train(student_train_list, student_image_dir, transform=transforms.Compose([RandomErasing(), RandomHorizontalFlip(),  ToTensor(), Cutout(), Normaliztion()]))
		...
		for i, sample_batched_teacher in enumerate(teacher_dataloader_train):
			for j, sample_batched_student in enumerate(student_dataloader_train):
				...
				if inputs_t.shape[0] != inputs_s.shape[0]:
					continue
				...
				# map_x, embedding, x_Block1, x_Block2, x_Block3, x_input =  model(inputs, inputs_ir, inputs_depth)
				#map_x, embedding, x_Block1, x_Block2, x_Block3, x_input =  model(inputs, inputs_depth)
				output_1, output_feat_1 = model_1(inputs_t)
				output_2, output_feat_2 = model_2(inputs_s)
				
				
				mmd_loss = MMDloss(output_feat_1, output_feat_2)
				similarity_loss = Simloss(output_feat_1, output_feat_2, spoof_label_t, spoof_label_s)
				# ce_loss = CEloss(output_2, torch.max(F.softmax(output_1), dim=1).values.data.long())
				ce_loss = CEloss(output_2, spoof_label_s.long().squeeze(1))
				# set_trace()
				# loss =  absolute_loss + contrastive_loss
				...
				# set_trace()
				train_accu.append(accuracy(output_2, spoof_label_s.long().squeeze(1))[0].item())
		...
		# scheduler.step()  
		# whole epoch average
		print('epoch:%d, Train: CE_loss= %.4f, MMD_loss= %.4f, Sim_loss= %.4f, accuracy= %.4f' % (epoch + 1, loss_CE.avg, loss_MMD.avg, loss_Sim.avg, sum(train_accu)/len(train_accu)))
		log_file.write('epoch:%d, Train: CE_loss= %.4f, MMD_loss= %.4f, Sim_loss= %.4f, accuracy= %.4f' % (epoch + 1, loss_CE.avg, loss_MMD.avg, loss_Sim.avg, sum(train_accu)/len(train_accu)))
		log_file.flush()
		...
		with torch.no_grad():
			...
			###########################################
			# val for threshold
			val_data = casia_fasd_dataset_val(student_val_list, student_image_dir, transform=transforms.Compose([Resize_val(256), CenterCrop_val(224), Normaliztion_valtest(), ToTensor_valtest()]))
			dataloader_val = DataLoader(val_data, batch_size=1, shuffle=False, num_workers=0)
